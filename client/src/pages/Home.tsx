...
import { Loader2, Inbox, Archive, Clock, Truck } from "lucide-react";
...
export default function Home() {
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center h-[60vh] text-muted-foreground">
          ...
          <p className="font-display tracking-widest uppercase">Загрузка данных логистики...</p>
        </div>
      );
    }
      ...
      return (
        ...
        >
          <div className="bg-card rounded-xl p-6 shadow-sm border border-border">
            <h2 className="text-xl font-bold font-display mb-6 border-b pb-2">Новая заявка</h2>
            <CreateRequestForm />
          </div>

          <div className="space-y-4">
            <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Последние заявки</h3>
        ...
        <div className="max-w-md mx-auto h-full">
          <Tabs defaultValue="new" className="w-full">
            <TabsList className="grid w-full grid-cols-3 mb-6 bg-muted/50 p-1">
              ...
              <TabsTrigger value="active" className="data-[state=active]:bg-white">В работе</TabsTrigger>
              <TabsTrigger value="history" className="data-[state=active]:bg-white">Архив</TabsTrigger>
            </TabsList>

            <AnimatePresence mode="wait">
              <TabsContent value="new" className="space-y-4 focus-visible:outline-none">
                {newRequests.length === 0 ? (
                  <EmptyState icon={Inbox} text="Новых заявок нет" />
                ) : (
                  newRequests.map(req => (
              ...
              <TabsContent value="active" className="space-y-4 focus-visible:outline-none">
                {inProgressRequests.length === 0 ? (
                  <EmptyState icon={Clock} text="Нет активных доставок" />
                ) : (
              ...
              <TabsContent value="history" className="space-y-4 focus-visible:outline-none">
                {completedRequests.length === 0 ? (
                  <EmptyState icon={Archive} text="История пуста" />
                ) : (
    ...
    if (role === "driver") {
      const myDeliveries = requests?.filter(r => r.status === "in_progress") || [];
      ...
      return (
        <div className="max-w-md mx-auto space-y-6">
          <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 mb-6">
            <h3 className="text-primary font-bold font-display uppercase text-sm tracking-wider mb-1">Активный маршрут</h3>
            <p className="text-2xl font-bold">{todayDeliveries.length} остановок сегодня</p>
          </div>

          <div className="space-y-4">
            <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest pl-1">Задачи</h3>
            {myDeliveries.length === 0 ? (
              <EmptyState icon={Truck} text="Нет назначенных доставок" />
            ) : (
